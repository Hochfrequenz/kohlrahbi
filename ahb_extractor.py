        ...
        for table in doc.tables[1:20]:
            ...
            for row in range(len(table.rows)):
                ...
                if len(table.row_cells(row)) == 3:
                    if (
                        row_cell_texts_as_list[0]
                        and not row_cell_texts_as_list[1]
                        and not row_cell_texts_as_list[2]
                        # and not row_cell_texts_as_list[3]
                    ):
                        dataframe_row[0] = row_cell_texts_as_list[0]
                        df.loc[df_row_index] = dataframe_row
                        df_row_index = df_row_index + 1
                        continue

                    for column_number in [0, 1, 2]:
                        ...
                        if column_number == 1:
                            ...
                            if cell.paragraphs[0].paragraph_format.left_indent == 36830:
                                ...
                                if "\n" in cell.text:
                                    ...
                                    if len(tabs) == 4 and cell.text.count("\t") == 5:
                                ...
                                if len(tabs) == 4 and cell.text.count("\t") == 4 and not "\n" in cell.text:
                                    splitted_text: List = cell.text.split("\t")
                                    (
                                        dataframe_row[3],
                                        dataframe_row[4],
                                        dataframe_row[5],
                                ...
                                if len(tabs) == 4 and cell.text.count("\t") == 4 and "\n" in cell.text:
                                    splitted_text = cell.text.split("\t")
                                    wrapped_lines = splitted_text[-1].split("\n")
                                    splitted_text[1] = splitted_text[1] + " " + " ".join(wrapped_lines[1:])
                                    splitted_text[4] = wrapped_lines[0]
                                    (
                                        dataframe_row[3],
                                        dataframe_row[4],
                                        dataframe_row[5],
